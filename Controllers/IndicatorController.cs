...
{
    static class IndicatorController
    {
        #region Field and properties

        static List<MovingAvarage> emas = MovingAvarage.GetEMA(ModelController.Klines, 21);
        static List<AdaptiveEnvelope> envelopes = AdaptiveEnvelope.GetAdaptiveEnvelope(
            ModelController.Klines, emas, 2, 2);

        #endregion

        #region Methods

        public static bool CheckMarketForShort()
        {
            ...
            var lastKline = ModelController.Klines.LastOrDefault();
            ...
            if (ModelController.Orders.ValidateNotNull != null)
            {
                sellOrders = from o in ModelController.Orders where o.Side == OrderSide.Buy select o;
        ...
        public static bool CheckMarketForLong()
        {
            bool positiveSignal = false;
            ...
            if (ModelController.Orders.ValidateNotNull != null)
            {
                buyOrders = from o in ModelController.Orders where o.Side == OrderSide.Buy select o;
        ...
        static void DataUpdate()
        {
            ...
            emas = MovingAvarage.GetEMA(ModelController.Klines, 21);

            //Creating list of adaptive envelopes
            envelopes = AdaptiveEnvelope.GetAdaptiveEnvelope(ModelController.Klines, emas, 2, 2);
        }
        ...
        //    ModelController.Klines = ModelController.Klines.FindAll(k => k.OpenTime >= DateTime.Now - new TimeSpan(90, 0, 0, 0));

        //    //Here we use list of our interface for better analitics
        //    List<IPosition> positions = new List<IPosition>();

        //    foreach (Kline k in ModelController.Klines)
        //    {
        //        decimal lowEnvelope;
